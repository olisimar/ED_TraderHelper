...
public class testFindBestTradeBasedOnInitialLoad {
	...
	public void verifyDataIsAvailable() {
		try {
			...
			stations = new ParseListings().parseListings(Files.readFile(new File(systemData.getWorkingDirectory() + "\\rawData\\listings.csv")), stations);
			...
			for(Entry<Long, Station> item: stations.entrySet()) {
				...
				if(system != null) {
					...
					numFailedLoads++;
					System.out.println(item.getValue().getBaseName() +"["+ item.getValue().getId()+"] did not find it's system id of "+ item.getValue().getSystemId());
				}
			}
			...
			/*
			for( Entry<Long, StarSystem> item : userData.getCurrentBubble().entrySet()) {
				System.out.println(item.getKey() +" : "+ item.getValue().getName() +" [No# Stations: "+ item.getValue().getStations().size()+ "] with the distance of "+ userData.getCurrentSystem().getPosition().distanceTo(item.getValue().getPosition()) );
			}
			
			for( BaseCommodity item: userData.getCurrentStation().getExportCommodities()) {
				System.out.println(item.getName() +" : B:"+ item.getBuyingPrice() +"\tS:"+ item.getSellingPrice());
			}
			System.out.println("  Sold items:");
			for( BaseCommodity item : userData.getCurrentStation().getSoldCommodities()) {
				System.out.println(item.getName() +" :  B:"+ item.getBuyingPrice() +" S:"+ item.getSellingPrice());
			}
			System.out.println("\n  Bought items:");
			for( BaseCommodity item : userData.getCurrentStation().getBoughtCommodities()) {
				System.out.println(item.getName() + " :  B:"+ item.getBuyingPrice() +"  S:"+ item.getSellingPrice());
			}
			*/
			...
			while(counter < 30) {
				if(counter < 10) {
					System.out.println(" "+ counter +" >> "+ calc.getPossibleTrades().pollFirst());
				} else {
					System.out.println(counter +" >> "+ calc.getPossibleTrades().pollFirst());
				}
				counter++;
			...
			/*
			for(TradeResult result : calc.getPossibleTrades()) {
				System.out.println(result.getCommodity().getName() +" : "+ result.getFromStation().getBaseName() +" -> "+ result.getToStation().getBaseName() +" [Profit: "+ result.getExpectedProfit() +"]");
			}
			*/
